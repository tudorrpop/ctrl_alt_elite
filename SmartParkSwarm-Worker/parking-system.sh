...
setup_parkinglot() {
    ...
    response=$(curl -s -w "%{http_code}" --location "http://$ip/api/setup" \
...
get_parkingspots() {
    ...
    curl -s --location "http://$ip/api/ParkingSpot" | \
...
vehicle_enter() {
    ...
    response=$(curl -s -w "%{http_code}" --location "http://$ip/api/VehicleEntry" \
    --header 'Content-Type: application/json' \
    --data "{
...
vehicle_exit() {
    ...
    response=$(curl -s -w "%{http_code}" --location --request PUT "http://$ip/api/VehicleEntry/exit/$qr_code" -o /dev/null)
